...
def test_connect_info_requires_windows():
    """connect info should fail on non-Windows platform."""
    result = runner.invoke(app, ["connect", "info"])
    assert result.exit_code == 1
    assert "AlphaCAM CLI requires Windows" in result.stderr


def test_drawing_create_requires_windows():
    result = runner.invoke(app, ["drawing", "create"])
    assert result.exit_code == 1
    assert "AlphaCAM CLI requires Windows" in result.stderr


def test_tool_list_requires_windows():
    result = runner.invoke(app, ["tool", "list"])
    assert result.exit_code == 1
    assert "AlphaCAM CLI requires Windows" in result.stderr


def test_mill_rough_requires_windows():
    result = runner.invoke(app, ["mill", "rough"])
    assert result.exit_code == 1
    assert "AlphaCAM CLI requires Windows" in result.stderr


def test_nc_output_requires_windows():
    result = runner.invoke(app, ["nc", "output", "test.nc"])
    assert result.exit_code == 1
    assert "AlphaCAM CLI requires Windows" in result.stderr


def test_batch_requires_windows():
    result = runner.invoke(app, ["batch", "process", "."])
    assert result.exit_code == 1
    assert "AlphaCAM CLI requires Windows" in result.stderr


def test_nest_run_requires_windows():
    result = runner.invoke(app, ["nest", "run", "test.csv"])
    assert result.exit_code == 1
    assert "AlphaCAM CLI requires Windows" in result.stderr


def test_post_list_requires_windows():
