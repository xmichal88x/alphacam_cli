...
try {
    ...
    $cmd2.CommandText = "SELECT CDMOrderDetailID FROM CDM_OrderDetails WHERE fkJobDetailID = $jdId ORDER BY CDMOrderDetailID ASC"
    $r2 = $cmd2.ExecuteReader()
    $ids = New-Object System.Collections.Generic.List[int]
    while ($r2.Read()) { $ids.Add([int]$r2.GetValue(0)) }
    $r2.Close()
    $flags = $Values -split ';'
    $start = $ids.Count - $flags.Count
    if ($start -lt 0) {
        Write-Error "row count mismatch: $($ids.Count) rows vs $($flags.Count) values"
        exit 1
    }
    if ($start -gt 0) {
        $ids = $ids.GetRange($start, $flags.Count)
    }
    $updated = 0
