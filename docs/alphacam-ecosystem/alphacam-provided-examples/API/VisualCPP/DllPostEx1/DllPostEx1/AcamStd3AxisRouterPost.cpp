...
CString AcamStd3AxisRouterPost::GetPostName()
{
	return _T("Example 3-Axis Router");
}

void AcamStd3AxisRouterPost::OutputFileLeadingLines(IPostDataPtr pPD)	// $10
{
...
void AcamStd3AxisRouterPost::OutputProgramLeadingLines(IPostDataPtr pPD)	// $12
{
    POST(":[PROGNUM]")
...
void AcamStd3AxisRouterPost::OutputProgramTrailingLines(IPostDataPtr pPD)	// $15
{
    POST("N[N] M30")
}
void AcamStd3AxisRouterPost::OutputFileTrailingLines(IPostDataPtr pPD)	// $17
{
	POST("%")
}
void AcamStd3AxisRouterPost::OutputRapid(IPostDataPtr pPD)	// $20, 21, 25
{
...
void AcamStd3AxisRouterPost::OutputUp(IPostDataPtr pPD)	// $30
{
}
void AcamStd3AxisRouterPost::OutputDown(IPostDataPtr pPD)	// $35
{
}
void AcamStd3AxisRouterPost::OutputFeed(IPostDataPtr pPD)	// $40, 50, 60
{
...
void AcamStd3AxisRouterPost::OutputCancelTool(IPostDataPtr pPD)	// $70
{
	POST("N[N] M09")
}
void AcamStd3AxisRouterPost::OutputSelectTool(IPostDataPtr pPD)	// $80
{
	POST("N[N] T[T][OFS] [ROT]       'Select tool and offset")
   POST("N[N] S[S] H[OFS] M06       'Next tool is [NT], Next XY is [NX], [NY]")
	first_rapid = true;
}
void AcamStd3AxisRouterPost::OutputSelectWorkPlane(IPostDataPtr pPD)	// $88
{
	POST("N[N] (WORK PLANE NAME = [WPN], OFFSET CODE = [WPO])")
}
void AcamStd3AxisRouterPost::OutputSelectToolAndWorkPlane(IPostDataPtr pPD)	// $89
{
	POST("' Change Tool and Work Plane at same time")
}
void AcamStd3AxisRouterPost::OutputCallSub(IPostDataPtr pPD)	// $90
{
	POST("N[N] M98 P[SN]                  'CALL SUB [SN]")
}
void AcamStd3AxisRouterPost::OutputBeginSub(IPostDataPtr pPD)	// $100
{
	POST(":[SN]                           'BEGIN SUB [SN]")
}
void AcamStd3AxisRouterPost::OutputEndSub(IPostDataPtr pPD)	// $110
{
	POST("N[N] M99                        'END SUB [SN]")
}
void AcamStd3AxisRouterPost::OutputOriginShift(IPostDataPtr pPD)	// $120
{
	POST("N[N] G52 X[OX] Y[OY]            'ORIGIN SHIFT")
}
void AcamStd3AxisRouterPost::OutputCancelOriginShift(IPostDataPtr pPD)	// $130
{
	POST("N[N] G52 X0.0 Y0.0              'CANCEL ORIGIN SHIFT")
}
void AcamStd3AxisRouterPost::OutputMoveClamp(IPostDataPtr pPD)	// $133
{
    pPD->ModalOff("");
    POST("'Move clamp# [MCN], X[CAX] Y[CAY], Z[CAZ], C[CLA] ([CLN])")
}
void AcamStd3AxisRouterPost::OutputDrillCycleCancel(IPostDataPtr pPD)	// $200
{
	POST("N[N] M09    ''Turn coolant OFF")
	POST("N[N] G80")
}
void AcamStd3AxisRouterPost::OutputFirstHoleSub(IPostDataPtr pPD)	// $205
{
   POST("N[N] X[AX] Y[AY]")
}
void AcamStd3AxisRouterPost::OutputNextHoleSub(IPostDataPtr pPD)	// $206
{
	POST("N[N] X[AX] Y[AY]")
}
void AcamStd3AxisRouterPost::OutputDrillCycleFirstHole(IPostDataPtr pPD)	// $210, 214, 220, 224, 230, 234, 240, 244
{
...
void AcamStd3AxisRouterPost::OutputDrillCycleNextHoles(IPostDataPtr pPD)	// $211, 215, 221, 225, 231, 235, 241, 245
{
    POST("N[N] X[AX] Y[AY]")
}
void AcamStd3AxisRouterPost::OutputDrillCycleSubParameters(IPostDataPtr pPD)	// $212, 216, 222, 226, 232, 236, 242, 246
{
	CString g9899 = pPD->DrillRapidAtRPlane ? _T("G99") : _T("G98");
	switch(pPD->DrillType)
	{
...
void AcamStd3AxisRouterPost::AfterOpenPost(IPostConfigurePtr pPC)
{
...
//int AcamStd3AxisRouterPost::FilterFunction(const char *buf, char **buf_new)
//{
//	CStringA str = buf;
//	str = "[" + str + "]";
//	*buf_new = _strdup(str);
